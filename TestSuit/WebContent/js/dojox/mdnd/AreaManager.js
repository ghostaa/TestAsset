	...
	null,
{
	...
	onDragStart: function(/*DOMNode*/node, /*Object*/coords, /*Object*/size){
		...
		if(this._sourceIndexArea < 0) return;
	...
	onBeforeDrop: function(/*DOMNode*/node){
	...
	onDrop: function(/*DOMNode*/node){
		...
		var originalTargetArea = this._areaList[this._currentIndexArea];
		var originalIndexChild = this._currentDropIndex;
		if(originalIndexChild == -1){
			originalIndexChild = originalTargetArea.items.length;
		}
		
		this.onBeforeDrop(node);
		//dropCancel
		
		this.onDropCancel(node, originalTargetArea, originalIndexChild);
