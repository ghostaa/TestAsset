 ...
 * 
 */
public class MyTestPaginationSortingNegListNameOp extends BTTServerOperation {
	...
	public int getPageData(int start, int count, List<SortColumn> sortColumnList) throws Exception {
		IndexedCollection tableData = (IndexedCollection) this.getContext().getKeyedCollection().getElementAt("rows");
		tableData.removeAll();

		// fetch data from back end service
		int totalSize = PageDataResource.SP04NEGLISTNAME.size();
		List<HashMap<String, Comparable<?>>> subList = PageDataResource.SP04NEGLISTNAME.subList(start,
