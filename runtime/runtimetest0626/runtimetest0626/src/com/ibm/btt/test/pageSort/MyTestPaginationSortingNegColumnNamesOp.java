 ...
 * 
 */
public class MyTestPaginationSortingNegColumnNamesOp extends BTTServerOperation {
	...
	public int getPageData(int start, int count, List<SortColumn> sortColumnList) throws Exception {
		...
		List<HashMap<String, Comparable<?>>> subList = PageDataResource.SP01DATA.subList(start,
		...
		for (int i = 0; (i < subList.size()); i++) {
			...
			kcoll.setValueAt("currencyNeqField", hm.get(TypeDataRowComparator.CURRENCY));
			kcoll.setValueAt("dateNeqField", hm.get(TypeDataRowComparator.DATE));
			kcoll.setValueAt("numberNeqField", hm.get(TypeDataRowComparator.NUMBER));
			kcoll.setValueAt("stringField", hm.get(TypeDataRowComparator.STRING));
			kcoll.setValueAt("plainNeqField", hm.get(TypeDataRowComparator.PLAIN));
